...
def load_multimodal_features_from_h5(feat_h5_video, feat_h5_audio, feature_names_list, 
                                     video_id, start, end, duration, get_full_feat=False):  #video_id:'v_5iQskA8713 '
    ...
    if 'vggish_features' in feature_names_list:
        audio_stack = feat_h5_audio.get(f'{video_id}/vggish_features')  #到H5中找声音索引
        ...
        T_audio, D_audio = audio_stack.shape    #返回音频矩阵shape

    if 'i3d_features' in feature_names_list:
        video_stack_rgb = feat_h5_video.get(f'{video_id}/i3d_features/rgb')     #rgb应该是画面
        video_stack_flow = feat_h5_video.get(f'{video_id}/i3d_features/flow')   #flow应该是光流
        
        assert video_stack_rgb.shape == video_stack_flow.shape
        T_video, D_video = video_stack_rgb.shape    #返回视屏矩阵的shape

        if T_video > T_audio:   #
            video_stack_rgb = video_stack_rgb[:T_audio, :]      #获得矩阵值!!!!!! （不同时间段）
        ...
        if get_full_feat == False:
            start_quantile = start / duration   #start和duration都为帧数，start_quantile结果返回多少秒
            ...
            if start_idx == end_idx:
                ...
                # [T:T] -> [T:T+1]
                    ...
                    end_idx += 1
            video_stack_rgb = video_stack_rgb[start_idx:end_idx, :] #把初始和结尾的索引放进去，找到视频片段
        ...
        return video_stack_rgb, video_stack_flow, audio_stack       #返回了画面、背景声音、说话声
...
def caption_iterator(start_token, end_token, pad_token, train_meta_path, val_1_meta_path,
    ...
    CAPTION = data.ReversibleField(     #caption不用动
    ...
    if use_imgs:
        IMGS = data.ReversibleField(  ##！！！！！！！！！！！！！！这个需要动 TODO 这里出大问题了
            tokenize='spacy', init_token=start_token,
            eos_token=end_token, pad_token=pad_token, lower=True,
            batch_first=True
        )
...
#生成train的标签         #subs也在这里
    ...
    if use_subs:        #不能累加,需要在外面做预处理，将subs和imgs合并生成相应的vocab
        SUBS.build_vocab(dataset.subs, min_freq=min_freq)
        train_subs_vocab = SUBS.vocab
    if use_imgs:        #TODO改到这里了  之前没有build_vocab
        IMGS.build_vocab(dataset.imgs, min_freq=min_freq)
        train_imgs_vocab = IMGS.vocab
    if phase == 'val_1':
        dataset = data.TabularDataset(
    ...
    return train_vocab, train_subs_vocab, datasetloader, train_imgs_vocab         #train_vocab为caption是一样的
#datasetloader在外面的名字叫caption_loader，所以我断定没有任何关联(分批处理)
...
class AudioVideoFeaturesDataset(Dataset):
    ...
    def getitem_1_stream_video(self, indices):
        ...
        for idx in indices:
            idx = idx.item()
            video_id, caption, start, end, duration, category, _, _, _ = self.dataset.iloc[idx] #到csv中找到片段
    ...
    def getitem_2_stream_video(self, indices):
        ...
        #找到合适的索引号！！！！！不一定从头来    应该是有一个时间序列的列表
        # [3]
        for idx in indices:
            idx = idx.item()
            video_id, caption, start, end, duration, category, _, _, _, _ = self.dataset.iloc[idx]
            
            # load the features from the hdf5
            vid_stack_rgb, vid_stack_flow, aud_stack = load_multimodal_features_from_h5(
                self.feat_h5_video, self.feat_h5_audio, self.feature_names_list, 
                video_id, start, end, duration, self.get_full_feat
            )   #返回了画面、背景声音、说话声         video_id为索引
...
class ActivityNetCaptionsIteratorDataset(Dataset):
    ...
    def __init__(self, start_token, end_token, pad_token, min_freq, batch_size,
        ...
        if modality == 'subs_audio_video' or modality == 'imgs_subs_audio_video' :
            self.use_subs = True
        else:
            self.use_subs = False
        if modality == 'imgs_subs_audio_video':
            self.use_imgs = True
        else:
            self.use_imgs = False
        # caption dataset *iterator*
        self.train_vocab, self.train_subs_vocab, self.caption_loader, self.train_imgs_vocab = caption_iterator(    #放
            start_token, end_token, pad_token, train_meta_path, val_1_meta_path, 
            val_2_meta_path, min_freq, batch_size, device, phase, use_categories,
            self.use_subs, self.use_imgs
        )
        # print("len(self.caption_loader):", len(self.caption_loader))
        ...
        if modality == 'audio_video' or modality == 'subs_audio_video' or modality == 'imgs_subs_audio_video':     #如果类型是视频+声音
            ...
            if modality == 'subs_audio_video':
                ...
                self.imgs_voc_size = None
            if modality == 'imgs_subs_audio_video':
                print(len(self.train_subs_vocab))
                self.subs_voc_size = len(self.train_subs_vocab) #把token的尺寸传入
                self.imgs_voc_size = len(self.train_imgs_vocab)
        else:
